...
def map_list():
    with open("data/status.txt", "r") as statusResultFirst:
        status = statusResultFirst.read()
    if int(status) == 0:
        with open('data/data.txt', 'r', encoding='UTF-8') as file:
            file_content = file.read()
        with open("data/status.txt", "w") as statusResultFinal:
            statusResultFinal.write("1")
        ...
        with open('data/maplist.txt', 'w', encoding='UTF-8') as file:
            for a in file_list:
                file.write(a + '|')
        r = win32api.MessageBox(0, "有创意工坊地图的需要?", "Emmm", win32con.MB_YESNO)
        if r == win32con.IDYES:
            try:
                os.chdir(file_content)
                os.chdir("../../")
